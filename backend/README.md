...
- Thread artifacts can be enumerated through `GET /api/threads/{thread_id}/artifacts`; the response contains user-scoped output metadata and forced-download URLs backed by the existing artifact file endpoint. Project-file downloads continue to support both files stored in the project workspace and files generated in an associated thread.
