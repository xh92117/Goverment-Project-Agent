...
def _build_local_thread_app() -> FastAPI:
    """Build a local-mode app without auth context but with the real store.

    This mirrors ``GATEWAY_ENABLE_LOCAL_AUTH=false``: no auth middleware stamps
    ``deerflow.runtime.user_context``, so router code must pass an explicit
    effective user id to ThreadMetaStore calls instead of relying on AUTO.
    """
    app = FastAPI()
    store = InMemoryStore()
    app.state.store = store
...
def test_create_and_search_threads_work_without_auth_context() -> None:
    app = _build_local_thread_app()

    with TestClient(app) as client:
        created = client.post("/api/threads", json={"metadata": {"title": "local"}})
        searched = client.post("/api/threads/search", json={"limit": 10})

    assert created.status_code == 200, created.text
    assert searched.status_code == 200, searched.text
    assert searched.json()[0]["thread_id"] == created.json()["thread_id"]
