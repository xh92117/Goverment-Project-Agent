...
Only AGENT_BASE_HOME is redirected to tmp_path for filesystem isolation.
...
def e2e_env(tmp_path, monkeypatch):
    ...
    - AGENT_BASE_HOME -> tmp_path (all thread data lands in a temp dir)
    - AGENT_BASE_PROJECT_ROOT -> repository root (shared skills/config assets
    ...
    monkeypatch.setenv("AGENT_BASE_HOME", str(tmp_path))
    monkeypatch.setenv(
...
class TestConfigManagement:
    ...
    def test_get_skill_found(self, e2e_env):
        ...
        c = DeerFlowClient(checkpointer=None, thinking_enabled=False)
        # 'bootstrap' is a minimal built-in public skill kept for agent-base
    ...
    def test_update_mcp_config_writes_and_invalidates(self, e2e_env, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AGENT_BASE_EXTENSIONS_CONFIG_PATH", str(config_file))
