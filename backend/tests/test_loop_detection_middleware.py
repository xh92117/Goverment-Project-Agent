...
class TestToolFrequencyDetection:
    ...
    def test_freq_hard_stop_at_limit(self):
        ...
        assert "未能生成完整最终回答" in msg.content

    def test_freq_hard_stop_records_visible_fallback_message(self):
        mw = LoopDetectionMiddleware(tool_freq_warn=3, tool_freq_hard_limit=6)
        journal = MagicMock()
        runtime = _make_runtime()
        ...
        mw._apply(_make_state(tool_calls=[self._read_call("/file_5.py")]), runtime)

        journal.record_assistant_message.assert_called_once()
        content = journal.record_assistant_message.call_args.args[0]
        assert "未能生成完整最终回答" in content
