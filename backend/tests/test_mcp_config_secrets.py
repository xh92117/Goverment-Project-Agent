...
def _clear_stdio_allowlist_env(monkeypatch) -> None:
...
@pytest.mark.asyncio
async def test_mcp_config_allows_management_when_local_auth_disabled(monkeypatch):
    """Single-user local deployments run with built-in auth disabled by default."""
    monkeypatch.setattr(
        "app.gateway.routers.mcp.get_gateway_config",
        lambda: SimpleNamespace(enable_local_auth=False),
    )

    await _require_admin_user(_request_without_user())


def test_validate_mcp_update_allows_default_npx_stdio_command(monkeypatch):
    _clear_stdio_allowlist_env(monkeypatch)
...
def test_validate_mcp_update_falls_back_to_legacy_stdio_allowlist(monkeypatch):
    _clear_stdio_allowlist_env(monkeypatch)
    monkeypatch.setenv(_LEGACY_MCP_STDIO_COMMAND_ALLOWLIST_ENV, "python")
    request = McpConfigUpdateRequest(
        mcp_servers={
            "python-mcp": McpServerConfigResponse(
                type="stdio",
...
def test_validate_mcp_update_prefers_agent_base_stdio_allowlist(monkeypatch):
    _clear_stdio_allowlist_env(monkeypatch)
    monkeypatch.setenv(_MCP_STDIO_COMMAND_ALLOWLIST_ENV, "python")
    monkeypatch.setenv(_LEGACY_MCP_STDIO_COMMAND_ALLOWLIST_ENV, "ruby")
    request = McpConfigUpdateRequest(
        mcp_servers={
            "ruby-mcp": McpServerConfigResponse(
                type="stdio",
                command="ruby",
                args=["trusted_mcp_server.rb"],
            )
        }
    )
    ...
    assert exc_info.value.status_code == 400
    assert "AGENT_BASE_MCP_STDIO_COMMAND_ALLOWLIST" in exc_info.value.detail
