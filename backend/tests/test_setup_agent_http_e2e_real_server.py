...
def isolated_deer_flow_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    """Stand up an isolated Agent Base data root + config under tmp_path.

    - Sets ``AGENT_BASE_HOME`` so paths land under tmp_path, not the real
    ...
    - Stages a copy of the project's ``config.yaml`` (or ``config.example.yaml``
      ...
      ``AGENT_BASE_CONFIG_PATH`` to it, so lifespan boot doesn't depend on the
    ...
    """
    home = tmp_path / "agent-base-home"
    home.mkdir()
    monkeypatch.setenv("AGENT_BASE_HOME", str(home))
    ...
    # config to tmp_path and pin ``AGENT_BASE_CONFIG_PATH`` to it.
    staged_config = tmp_path / "config.yaml"
    staged_config.write_text(_MINIMAL_CONFIG_YAML, encoding="utf-8")
    monkeypatch.setenv("AGENT_BASE_CONFIG_PATH", str(staged_config))
...
def isolated_app(isolated_deer_flow_home: Path, monkeypatch: pytest.MonkeyPatch):
    """Build a fresh FastAPI app inside a clean AGENT_BASE_HOME.
    ...
    # Re-resolve the config from the test-only AGENT_BASE_HOME and pin its
