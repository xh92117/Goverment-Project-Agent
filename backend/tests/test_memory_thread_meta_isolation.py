...
@pytest.mark.anyio
@pytest.mark.no_auto_user
async def test_no_context_uses_default_user_bucket(store):
    """No-auth calls use the documented default bucket without leaking other users."""
    await store.create("t-default", display_name="default thread")
    await store.create("t-other", display_name="other thread", user_id="other-user")

    results = await store.search()

    assert [row["thread_id"] for row in results] == ["t-default"]
    assert results[0]["user_id"] == "default"
