...
def _setup_auth(tmp_path):
    ...
    os.environ["GATEWAY_ENABLE_LOCAL_AUTH"] = "true"
    gateway_config._gateway_config = None
    asyncio.run(init_engine("sqlite", url=url, sqlite_dir=str(tmp_path), include_auth_models=True))
    ...
    try:
        ...
        deps._cached_repo = None
        gateway_config._gateway_config = None
        os.environ.pop("GATEWAY_ENABLE_LOCAL_AUTH", None)
        _SETUP_STATUS_CACHE.clear()
        _SETUP_STATUS_INFLIGHT.clear()
