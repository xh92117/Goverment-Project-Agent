...
def test_sqlite_round_trip_new_fields():
    ...
    and run_events use). The old separate .deer-flow/users.db file is gone.
    """
    import asyncio
    import tempfile
    ...
    async def _run() -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            await init_engine("sqlite", url=url, sqlite_dir=tmpdir, include_auth_models=True)
            try:
...
def test_update_user_raises_when_row_concurrently_deleted(tmp_path):
    ...
    async def _run() -> None:
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            await init_engine("sqlite", url=url, sqlite_dir=d, include_auth_models=True)
            try:
