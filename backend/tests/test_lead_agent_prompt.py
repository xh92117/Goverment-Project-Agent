...
def test_apply_prompt_template_includes_government_project_runtime(monkeypatch):
    config = SimpleNamespace(
        sandbox=SimpleNamespace(mounts=[]),
        skills=SimpleNamespace(container_path="/mnt/skills"),
    )
    ...
    prompt = prompt_module.apply_prompt_template(
        ...
        government_project_tools={"knowledge": False, "plan": True, "web": False},
        government_project_capabilities={"knowledgeRag": False, "webSearch": False},
        government_project_model_parameters={"temperature": 0.3, "maxTokens": 6000},
    )

    assert "<government_project_declaration_runtime>" in prompt
    assert "Knowledge-base retrieval: disabled" in prompt
    assert "Plan/Todo workflow: enabled" in prompt
    assert "Knowledge RAG: disabled" in prompt
    assert "Temperature: 0.3" in prompt
    assert "Max output tokens: 6000" in prompt
    assert "proposal_save_markdown" in prompt
    assert "knowledge_search_evidence" in prompt
    assert 'verification_statuses=["human_verified"]' in prompt
    assert "word_image_markdown" in prompt
    assert "guide/template analysis -> topic planning -> literature/status review -> research plan -> budget planning -> compliance review" in prompt
...
def test_apply_prompt_template_omits_government_project_runtime_for_other_agents(monkeypatch):
    ...
    prompt = prompt_module.apply_prompt_template(
        ...
        government_project_tools={"knowledge": True},
    )

    assert "<government_project_declaration_runtime>" not in prompt
