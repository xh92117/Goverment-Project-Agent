...
class TestChannelStore:
    ...
    def test_set_and_get_thread_id(self, store):
        store.set_thread_id("feishu", "ch1", "thread-abc", user_id="u1")
        assert store.get_thread_id("feishu", "ch1") == "thread-abc"

    def test_get_nonexistent_returns_none(self, store):
        assert store.get_thread_id("feishu", "nonexistent") is None

    def test_remove(self, store):
        store.set_thread_id("feishu", "ch1", "t1")
        assert store.remove("feishu", "ch1") is True
        assert store.get_thread_id("feishu", "ch1") is None
    ...
    def test_list_entries_all(self, store):
        store.set_thread_id("feishu", "ch1", "t1")
        store.set_thread_id("dingtalk", "ch2", "t2")
    ...
    def test_list_entries_filtered(self, store):
        store.set_thread_id("feishu", "ch1", "t1")
        store.set_thread_id("dingtalk", "ch2", "t2")
        entries = store.list_entries(channel_name="feishu")
        assert len(entries) == 1
        assert entries[0]["channel_name"] == "feishu"

    def test_persistence(self, tmp_path):
        path = tmp_path / "store.json"
        store1 = ChannelStore(path=path)
        store1.set_thread_id("feishu", "ch1", "t1")

        store2 = ChannelStore(path=path)
        assert store2.get_thread_id("feishu", "ch1") == "t1"
    ...
    def test_update_preserves_created_at(self, store):
        ...
        store.set_thread_id("feishu", "ch1", "t2")
        entries = store.list_entries()
...
class TestExtractResponseText:
    ...
    def test_clarification_tool_message(self):
        ...
        result = {
            "messages": [
                {"type": "human", "content": "鍋ヨ韩"},
                {"type": "ai", "content": "", "tool_calls": [{"name": "ask_clarification", "args": {"question": "鎮ㄦ兂浜嗚В鍝柟闈紵"}}]},
                {"type": "tool", "name": "ask_clarification", "content": "鎮ㄦ兂浜嗚В鍝柟闈紵"},
            ]
        }
        assert _extract_response_text(result) == "鎮ㄦ兂浜嗚В鍝柟闈紵"
...
class TestChannelManager:
    ...
    def test_handle_chat_uses_user_session_overrides(self):
        ...
        async def go():
            ...
            inbound = InboundMessage(channel_name="feishu", chat_id="chat1", user_id="vip-user", text="hi")
            await bus.publish_inbound(inbound)
            await _wait_for(lambda: len(outbound_received) >= 1)
            await manager.stop()
    ...
    def test_each_topic_creates_new_thread(self):
        """Messages with distinct topic_ids should each create a new Agent Base thread."""
        from app.channels.manager import ChannelManager
    ...
    def test_same_topic_reuses_thread(self):
        """Messages with the same topic_id should reuse the same Agent Base thread."""
        from app.channels.manager import ChannelManager
    ...
    def test_none_topic_reuses_thread(self):
        """Messages with topic_id=None should reuse the same thread (e.g. feishu private chat)."""
        from app.channels.manager import ChannelManager
        ...
        async def go():
            ...
            # Send two messages with topic_id=None (simulates feishu private chat)
...
class TestResolveRunParamsUserId:
    ...
    def test_safe_user_id_is_passed_through(self):
        ...
        msg = InboundMessage(channel_name="feishu", chat_id="c", user_id="123456", text="hi")
...
class TestFormatArtifactText:
    def test_single_artifact(self):
        ...
        assert text == "Created File: 馃搸 report.md"
    ...
    def test_multiple_artifacts(self):
        ...
        assert text == "Created Files: 馃搸 a.txt銆乥.csv"
...
class TestChannelService:
    ...
    def test_session_config_is_forwarded_to_manager(self):
        ...
        assert service.manager._channel_sessions["feishu"]["assistant_id"] == "mobile_agent"
        assert service.manager._channel_sessions["feishu"]["users"]["vip"]["assistant_id"] == "vip_agent"

    def test_service_urls_fall_back_to_legacy_env(self, monkeypatch):
        from app.channels.service import ChannelService
    ...
    def test_service_urls_prefer_agent_base_env(self, monkeypatch):
        from app.channels.service import ChannelService
    ...
    def test_config_service_urls_override_env(self, monkeypatch):
        ...
        monkeypatch.setenv("AGENT_BASE_CHANNELS_LANGGRAPH_URL", "http://gateway:8001/api")
        monkeypatch.setenv("AGENT_BASE_CHANNELS_GATEWAY_URL", "http://gateway:8001")
    ...
    def test_from_app_config_uses_explicit_config(self):
        ...
        assert service._config == {"feishu": {"enabled": False}}
    ...
    def test_disabled_channel_with_int_creds_emits_warning(self, caplog):
        ...
        async def go():
            ...
            service = ChannelService(
                channels_config={
                    "feishu": {"enabled": False, "app_id": 123456789},
                }
            )
            with caplog.at_level(logging.WARNING, logger="app.channels.service"):
                await service.start()
        ...
        _run(go())
        assert any("feishu" in r.message and r.levelno == logging.WARNING for r in caplog.records)
    ...
    def test_disabled_channel_without_creds_emits_info(self, caplog):
        ...
        _run(go())
        warning_records = [r for r in caplog.records if "feishu" in r.message and r.levelno == logging.WARNING]
