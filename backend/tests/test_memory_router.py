...
def test_import_memory_route_returns_imported_memory() -> None:
    ...
    imported_memory = _sample_memory(
        facts=[
            {
                "id": "fact_import",
                "content": "User works on Agent Base.",
                "category": "context",
                "confidence": 0.87,
                "createdAt": "2026-03-20T00:00:00Z",
                "source": "manual",
...
def test_update_memory_config_route_persists_and_reloads_config(tmp_path) -> None:
    app = FastAPI()
    app.include_router(memory.router)
    ...
    reloaded_config = SimpleNamespace(
        memory=MemoryConfig(enabled=True, injection_enabled=False, max_facts=100)
    )

    with (
        patch("app.gateway.routers.memory._require_admin_user", new=AsyncMock()),
        patch("app.gateway.routers.memory.AppConfig.resolve_config_path", return_value=config_path),
        patch("app.gateway.routers.memory.reload_app_config", return_value=reloaded_config) as reload_config,
    ):
        with TestClient(app) as client:
            response = client.patch(
                "/api/memory/config",
                json={"enabled": True, "injection_enabled": False},
            )

    assert response.status_code == 200
    assert response.json()["enabled"] is True
    assert response.json()["injection_enabled"] is False
    assert reload_config.call_args.args == (str(config_path),)
    persisted = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    assert persisted["memory"]["enabled"] is True
    assert persisted["memory"]["injection_enabled"] is False
    assert persisted["memory"]["max_facts"] == 100
