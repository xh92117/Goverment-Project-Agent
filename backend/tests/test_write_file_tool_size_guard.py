...
def test_env_override_raises_cap(monkeypatch: pytest.MonkeyPatch):
    """Setting AGENT_BASE_WRITE_FILE_MAX_BYTES lets deployments accept larger
    payloads when the underlying LLM/network can demonstrably handle them.
    """
    monkeypatch.setenv("AGENT_BASE_WRITE_FILE_MAX_BYTES", str(300 * 1024))
    payload = "a" * (150 * 1024)  # 150 KB — would normally trip the 80 KB cap
    result = _call_write_file(content=payload)
    assert result == "OK"


def test_env_override_falls_back_to_legacy_name(monkeypatch: pytest.MonkeyPatch):
...
def test_env_override_prefers_agent_base_name(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("AGENT_BASE_WRITE_FILE_MAX_BYTES", str(90 * 1024))
    monkeypatch.setenv("DEERFLOW_WRITE_FILE_MAX_BYTES", str(300 * 1024))
    payload = "a" * (150 * 1024)
    result = _call_write_file(content=payload)
    assert result.startswith("Error: write_file content")
...
def test_env_override_zero_disables_guard(monkeypatch: pytest.MonkeyPatch):
    ...
    """
    monkeypatch.setenv("AGENT_BASE_WRITE_FILE_MAX_BYTES", "0")
