...
class TestDatabaseConfig:
    def test_sqlite_default_uses_agent_base_filename(self, tmp_path, monkeypatch):
        monkeypatch.delenv("AGENT_BASE_DB_PATH", raising=False)
        monkeypatch.delenv("DEER_FLOW_DB_PATH", raising=False)

        c = DatabaseConfig(backend="sqlite", sqlite_dir=str(tmp_path))

        assert c.sqlite_path == str(tmp_path / "agent_base.db")
    ...
    def test_sqlite_paths_unified(self):
        ...
        assert c.sqlite_path.endswith("agent_base.db")
    ...
    def test_sqlite_path_prefers_existing_legacy_file_for_migration(self, tmp_path, monkeypatch):
        monkeypatch.delenv("AGENT_BASE_DB_PATH", raising=False)
        monkeypatch.delenv("DEER_FLOW_DB_PATH", raising=False)
        legacy_path = tmp_path / "deerflow.db"
        legacy_path.touch()

        c = DatabaseConfig(backend="sqlite", sqlite_dir=str(tmp_path))

        assert c.sqlite_path == str(legacy_path)

    def test_sqlite_path_prefers_agent_base_file_when_both_exist(self, tmp_path, monkeypatch):
        monkeypatch.delenv("AGENT_BASE_DB_PATH", raising=False)
        monkeypatch.delenv("DEER_FLOW_DB_PATH", raising=False)
        legacy_path = tmp_path / "deerflow.db"
        agent_base_path = tmp_path / "agent_base.db"
        legacy_path.touch()
        agent_base_path.touch()

        c = DatabaseConfig(backend="sqlite", sqlite_dir=str(tmp_path))

        assert c.sqlite_path == str(agent_base_path)

    def test_sqlite_path_env_override(self, tmp_path, monkeypatch):
        override = tmp_path / "custom.db"
        monkeypatch.setenv("AGENT_BASE_DB_PATH", str(override))
        monkeypatch.setenv("DEER_FLOW_DB_PATH", str(tmp_path / "legacy.db"))

        c = DatabaseConfig(backend="sqlite", sqlite_dir=str(tmp_path / "data"))

        assert c.sqlite_path == str(override)

    def test_sqlite_path_legacy_env_override(self, tmp_path, monkeypatch):
        override = tmp_path / "legacy.db"
        monkeypatch.delenv("AGENT_BASE_DB_PATH", raising=False)
        monkeypatch.setenv("DEER_FLOW_DB_PATH", str(override))
