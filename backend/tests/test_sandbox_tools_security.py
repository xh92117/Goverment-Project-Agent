...
_THREAD_DATA = {
    "workspace_path": "/tmp/agent-base/threads/t1/user-data/workspace",
    "uploads_path": "/tmp/agent-base/threads/t1/user-data/uploads",
    "outputs_path": "/tmp/agent-base/threads/t1/user-data/outputs",
}
...
def test_replace_virtual_path_maps_virtual_root_and_subpaths() -> None:
    assert Path(replace_virtual_path("/mnt/user-data/workspace/a.txt", _THREAD_DATA)).as_posix() == "/tmp/agent-base/threads/t1/user-data/workspace/a.txt"
    assert Path(replace_virtual_path("/mnt/user-data", _THREAD_DATA)).as_posix() == "/tmp/agent-base/threads/t1/user-data"
...
def test_replace_virtual_path_preserves_trailing_slash() -> None:
    ...
    assert result == "/tmp/agent-base/threads/t1/user-data/workspace/"
...
def test_replace_virtual_path_preserves_trailing_slash_windows_style() -> None:
    ...
    """
    win_thread_data = {
        "workspace_path": r"C:\agent-base\threads\t1\user-data\workspace",
        "uploads_path": r"C:\agent-base\threads\t1\user-data\uploads",
        "outputs_path": r"C:\agent-base\threads\t1\user-data\outputs",
    }
...
def test_replace_virtual_path_preserves_windows_style_for_nested_subdir_trailing_slash() -> None:
    ...
    assert result == "C:\\agent-base\\threads\\t1\\user-data\\workspace\\subdir\\"
    assert "/" not in result, f"Mixed separators in Windows path: {result!r}"
...
def test_replace_virtual_paths_in_command_preserves_trailing_slash() -> None:
    ...
    assert "/tmp/agent-base/threads/t1/user-data/workspace/" in result, f"Trailing slash lost in: {result!r}"
...
def test_mask_local_paths_in_output_hides_host_paths() -> None:
    output = "Created: /tmp/agent-base/threads/t1/user-data/workspace/result.txt"
    masked = mask_local_paths_in_output(output, _THREAD_DATA)

    assert "/tmp/agent-base/threads/t1/user-data" not in masked
...
def test_mask_local_paths_in_output_hides_skills_host_paths() -> None:
    ...
    with (
        ...
        patch("deerflow.sandbox.tools._get_skills_host_path", return_value="/home/user/agent-base/skills"),
    ):
        output = "Reading: /home/user/agent-base/skills/public/bootstrap/SKILL.md"
        masked = mask_local_paths_in_output(output, _THREAD_DATA)

        assert "/home/user/agent-base/skills" not in masked
...
def test_validate_local_tool_path_allows_user_data_paths() -> None:
    # Should not raise 鈥?user-data paths are always allowed
...
def test_resolve_skills_path_resolves_correctly() -> None:
    ...
    with (
        ...
        assert resolved == "/home/user/agent-base/skills/public/bootstrap/SKILL.md"
...
def test_resolve_skills_path_resolves_root() -> None:
    ...
    with (
        ...
        assert resolved == "/home/user/agent-base/skills"
...
def test_replace_virtual_paths_in_command_replaces_skills_paths() -> None:
    ...
    with (
        ...
        assert "/home/user/agent-base/skills/public/bootstrap/SKILL.md" in result
...
def test_replace_virtual_paths_in_command_replaces_both() -> None:
    ...
    with (
        ...
        assert "/tmp/agent-base/threads/t1/user-data/workspace/out.txt" in result
...
def test_replace_virtual_paths_in_command_replaces_acp_workspace() -> None:
    ...
    with patch("deerflow.sandbox.tools._get_acp_workspace_host_path", return_value=acp_host):
        ...
        assert "/tmp/agent-base/threads/t1/user-data/outputs/hello.py" in result
...
def test_apply_cwd_prefix_prepends_workspace() -> None:
    ...
    assert "/tmp/agent-base/threads/t1/user-data/workspace" in result
...
def test_str_replace_parallel_updates_in_isolated_sandboxes_should_not_share_path_lock(monkeypatch) -> None:
    ...
    def worker(runtime: SimpleNamespace, old_str: str, new_str: str) -> None:
        try:
            result = str_replace_tool.func(
                runtime=runtime,
                description="闅旂 sandbox 骞跺彂鏇挎崲鍚屼竴璺緞",
                path="/mnt/user-data/workspace/shared.txt",
                old_str=old_str,
                new_str=new_str,
            )
...
def test_write_file_tool_bounds_large_oserror_and_masks_local_paths(monkeypatch) -> None:
    ...
    assert "/tmp/agent-base/threads/t1/user-data/workspace" not in result
...
def test_write_file_tool_formats_all_other_failure_branches(
    ...
    result = write_file_tool.func(
        runtime=runtime,
        description="Validate error branch formatting",
        path="/mnt/user-data/workspace/output.txt",
        content="tiny payload",
    )
...
def test_file_operation_lock_memory_cleanup() -> None:
    ...
    test_path = "/tmp/agent-base/memory_leak_test_file.txt"
    lock_key = (MockSandbox.id, test_path)

    # 纭繚娴嬭瘯寮€濮嬪墠 key 涓嶅瓨鍦?    assert lock_key not in _FILE_OPERATION_LOCKS
    ...
    # 妫€鏌ョ壒瀹?key 鏄惁琚竻鐞嗭紙鑰屼笉鏄鏌ユ€婚暱搴︼級
