...
def test_entrypoint_excludes_runtime_state_from_uvicorn_reload():
    ...
    assert ': "${AGENT_BASE_HOME:=/app/backend/.agent-base}"' in content
    assert "DEER_FLOW_HOME" not in content
    assert 'mkdir -p "$AGENT_BASE_HOME" /app/backend/.agent-base' in content
    ...
    assert '--reload-exclude="$AGENT_BASE_HOME"' in content
    assert "--reload-exclude=/app/backend/.agent-base" in content
