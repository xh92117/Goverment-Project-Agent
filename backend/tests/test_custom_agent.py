...
class TestLoadAgentConfig:
    ...
    def test_user_agent_dir_without_config_falls_back_to_legacy_config(self, tmp_path):
        """A user-scoped runtime artifact directory must not shadow a legacy agent config."""
        legacy_dir = tmp_path / "agents" / "government-project-declaration"
        legacy_dir.mkdir(parents=True)
        (legacy_dir / "config.yaml").write_text(
            "name: government-project-declaration\ndescription: shared config\n",
            encoding="utf-8",
        )
        user_dir = tmp_path / "users" / "default" / "agents" / "government-project-declaration"
