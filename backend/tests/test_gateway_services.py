...
def test_context_merges_into_configurable():
    ...
    assert config["configurable"]["execution_mode"] == "deep"
...
def test_apply_execution_mode_budget_deep_mode():
    ...
    assert config["configurable"]["execution_mode"] == "deep"
    assert config["context"]["execution_mode"] == "deep"
    assert config["configurable"]["subagent_enabled"] is True
...
def test_apply_execution_mode_budget_preserves_larger_explicit_limits():
    ...
    config = build_run_config(
        "thread-1",
        {"recursion_limit": 250, "configurable": {"max_concurrent_subagents": 6}},
        None,
    )
    merge_run_context_overrides(config, {"execution_mode": "deep"})
    apply_execution_mode_budget(config, SimpleNamespace(execution_modes=ExecutionModesConfig()))
...
def test_merge_run_context_overrides_propagates_government_project_settings():
    """Government declaration frontend settings must reach the agent runtime."""
    from app.gateway.services import build_run_config, merge_run_context_overrides

    tools = {"knowledge": True, "plan": True, "web": False}
    capabilities = {"knowledgeRag": True, "webSearch": False}
    model_parameters = {"temperature": 0.4, "maxTokens": 6000}
    config = build_run_config("thread-1", None, None)

    merge_run_context_overrides(
        config,
        {
            "government_project_tools": tools,
            "government_project_capabilities": capabilities,
            "government_project_model_parameters": model_parameters,
            "output_language": "zh-CN",
            "output_style": "zh-CN government-project markdown",
            "knowledge_answer_policy": "synthesize before answering",
        },
    )

    assert config["configurable"]["government_project_tools"] == tools
    assert config["configurable"]["government_project_capabilities"] == capabilities
    assert config["configurable"]["government_project_model_parameters"] == model_parameters
    assert config["context"]["government_project_tools"] == tools
    assert config["context"]["government_project_capabilities"] == capabilities
    assert config["context"]["government_project_model_parameters"] == model_parameters
    assert config["configurable"]["output_language"] == "zh-CN"
    assert config["context"]["output_language"] == "zh-CN"
    assert config["configurable"]["output_style"] == "zh-CN government-project markdown"
    assert config["context"]["knowledge_answer_policy"] == "synthesize before answering"


def test_merge_run_context_overrides_propagates_project_identity():
    """Project chat runs must expose their project id/name to workspace tools."""
    from app.gateway.services import build_run_config, merge_run_context_overrides
    ...
    merge_run_context_overrides(
        config,
        {
            ...
            "project_name": "隧道衬砌检测项目",
            "thread_id": "ignored",
        },
    )

    assert config["configurable"]["project_id"] == "project-1"
    assert config["configurable"]["project_name"] == "隧道衬砌检测项目"
    assert config["context"]["project_id"] == "project-1"
    assert config["context"]["project_name"] == "隧道衬砌检测项目"
    assert "thread_id" not in config["context"]
