...
│   │   │   └── routers/       # FastAPI route modules (models, mcp, memory, skills, uploads, threads, artifacts, agents, channels)
...
- **App** (`app/`): Unpublished application code. Import prefix: `app.*`. Contains the FastAPI Gateway API and IM channel integrations (DingTalk, WeChat, WeCom, Feishu).
...
2. `AGENT_BASE_CONFIG_PATH` environment variable
3. `DEER_FLOW_CONFIG_PATH` environment variable (legacy fallback)
4. `config.yaml` in current directory (backend/)
...
by `use` and provider-specific endpoint fields such as `base_url`.
...
Configuration priority:
1. Explicit `config_path` argument
2. `AGENT_BASE_EXTENSIONS_CONFIG_PATH` environment variable
3. `DEER_FLOW_EXTENSIONS_CONFIG_PATH` environment variable (legacy fallback)
4. `extensions_config.json` in current directory (backend/)
5. `extensions_config.json` in parent directory (project root - **recommended location**)

### Gateway API (`app/gateway/`)

FastAPI application on port 8001 with health check at `GET /health`. Set `GATEWAY_ENABLE_DOCS=false` to disable `/docs`, `/redoc`, and `/openapi.json` in production (default: enabled). Built-in local auth is optional in Agent Base: set `GATEWAY_ENABLE_LOCAL_AUTH=true` to mount `/api/v1/auth`, initialize the `users` table, enable admin setup, and enforce owner isolation. Set frontend `NEXT_PUBLIC_ENABLE_LOCAL_AUTH=true` at the same time so Web routes use the login/setup flow.

CORS is same-origin by default when requests enter through nginx on port 2026. Split-origin or port-forwarded browser clients must opt in with `GATEWAY_CORS_ORIGINS` (comma-separated exact origins). When built-in local auth is enabled, Gateway `CORSMiddleware` and `CSRFMiddleware` both read that variable so browser CORS and auth-origin checks stay aligned.
...
| **Artifacts** (`/api/threads/{id}/artifacts`) | `GET /` - list up to 500 current-thread output files with metadata and download URLs; `GET /{path}` - serve artifacts; active content types (`text/html`, `application/xhtml+xml`, `image/svg+xml`) are always forced as download attachments to reduce XSS risk; `?download=true` still forces download for other file types. Listing resolves the authenticated user's thread output root and offloads recursive filesystem scanning from the event loop. |
| **Thread Runs** (`/api/threads/{id}/runs`) | `POST /` - create background run; `POST /stream` - create + SSE stream; `POST /wait` - create + block; `GET /` - list runs; `GET /{rid}` - run details; `POST /{rid}/cancel` - cancel; `GET /{rid}/join` - join SSE; `GET /{rid}/messages` - paginated messages `{data, has_more}`; `GET /{rid}/events` - full event stream; `GET /../messages` - thread messages; `GET /../token-usage` - aggregate tokens |
| **Runs** (`/api/runs`) | `POST /stream` - stateless run + SSE; `POST /wait` - stateless run + block; `GET /{rid}/messages` - paginated messages by run_id `{data, has_more}` (cursor: `after_seq`/`before_seq`) |
...
**Government-project heterogeneous experts**:
- `expert_contract.py` defines capability ownership/exclusions and a common evidence-bearing handoff format for the eight declaration experts; these names are capability boundaries, not role-playing personas.
- `task_tool` wraps expert assignments with `project_id`, `project_name`, and `applicant_id`; `SubagentExecutor` propagates safe runtime scope to expert tools.
- Decompose into orthogonal deliverables. Repeating one broad question across agents is allowed only as explicit high-risk independent verification; repeated model claims are not majority-vote truth.
- The proposal writer has no web tools and may only transform supplied decisions/evidence. The compliance reviewer is read-only (no draft-save tool) and uses a different configured model to reduce correlated errors.
- Lead synthesis resolves conflicts by authority/applicability and builds an evidence-to-claim matrix before writing; unresolved eligibility, budget, or technical-route conflicts must remain visible.
...
- Preserves `provider` for settings metadata but never forwards it to provider clients
...
Bridges external messaging platforms (DingTalk, WeChat, WeCom, Feishu) to the DeerFlow agent via Gateway's LangGraph-compatible API.
...
- `manager.py` - Core dispatcher: creates threads via `client.threads.create()`, routes commands, uses `client.runs.wait()` for final-response channels, and uses `client.runs.stream(["messages-tuple", "values"])` for Feishu incremental outbound updates
- `base.py` - Abstract `Channel` base class (start/stop/send lifecycle)
- `service.py` - Manages lifecycle of all configured channels from `config.yaml`
- `dingtalk.py` / `feishu.py` / `wechat.py` / `wecom.py` - Platform-specific implementations (`feishu.py` tracks the running card `message_id` in memory and patches the same card in place; `dingtalk.py` optionally uses AI Card streaming for in-place updates when `card_template_id` is configured)
...
5. WeChat chat: `runs.wait()` → extract final response → publish outbound
...
- In Docker Compose, IM channels run inside the `gateway` container, so `localhost` points back to that container. Use `http://gateway:8001/api` for `langgraph_url` and `http://gateway:8001` for `gateway_url`, or set `AGENT_BASE_CHANNELS_LANGGRAPH_URL` / `AGENT_BASE_CHANNELS_GATEWAY_URL` (`DEER_FLOW_CHANNELS_*` remains a legacy fallback).
- Per-channel configs: `dingtalk` (client_id, client_secret, optional `card_template_id` for AI Card streaming), `feishu` (app_id, app_secret), `wechat` (bot_token), `wecom` (bot_id, bot_secret)
...
- `project.py` - Strict project memory isolated at `{base_dir}/users/{user_id}/projects/{project_id}/memory.json`
...
**Government-project memory rules**:
- `government-project-declaration` requires `project_id`; without it, middleware skips both update and legacy-memory injection.
- Project memory contains `confirmedFacts`, `workingAssumptions`, and scalar `workflowState`. Automatic LLM extraction reads user-authored messages only and can write only `workingAssumptions`; any model-returned `confirmedFacts` field is ignored.
- Dynamic context injects `<project_memory>` with explicit verified-versus-unverified labels. Project tasks also carry `applicant_id`; cross-project fallback and cross-project aggregation are forbidden.
- Dream-memory distillation, offline clustering, and cross-project consolidation are not implemented in this phase.
...
| `langfuse_tags`       | `env:<AGENT_BASE_ENV>` + `model:<model_name>` |

Returns `{}` when Langfuse is not in the enabled providers — LangSmith-only deployments are unaffected. Set `AGENT_BASE_ENV` to tag traces by deployment environment; `DEER_FLOW_ENV` and `ENVIRONMENT` remain supported as fallbacks. Tests live in `tests/test_tracing_factory.py`, `tests/test_tracing_metadata.py`, `tests/test_worker_langfuse_metadata.py`, and `tests/test_client_langfuse_metadata.py`.
...
### Knowledge Image Evidence

The declaration knowledge base keeps binary image assets separate from the
text-oriented LLM-Wiki map. `deerflow.knowledge.assets` stores originals,
...
- Image evidence is scoped and deduplicated by `applicant_id`, starts in
  `needs_review`, and has applicant-validated content/thumbnail endpoints.
- `deerflow.knowledge.evidence_extraction` is the multimodal extraction
  boundary. It must select only a configured model with `supports_vision=true`;
  there is intentionally no Tesseract, pytesseract, filename-OCR, or text-model
  fallback.
- Image upload stores a `pending` evidence card without invoking a model.
  `build_knowledge_index_from_folder()` processes pending/failed cards before
  scanning text sources. The vision model autonomously decides relevance,
  evidence type, structured fields, OCR text, proposal chapters, and tags.
...
- `POST /api/knowledge/evidence/batch-review` accepts explicit evidence IDs and
  ...
  partially hiding the batch result.
- `POST /api/knowledge/evidence/{id}/extract` re-runs extraction. A transition
  to `human_verified` is rejected unless `holder` plus a certificate number,
  issuer, or date is present; the server records `reviewed_at` and `reviewed_by`.
- `knowledge_search_evidence` and `knowledge_read_evidence` expose structured
  cards without granting the generic `view_image` tool access to the knowledge
  root.
- Human-verified evidence reads expose a stable
  `evidence://<applicant_id>/<evidence_id>` URI and ready-to-use
  `word_image_markdown`. The declaration-agent prompt must proactively search
  with `verification_statuses=["human_verified"]` and place that Markdown in
  relevant drafts/appendices. `app.gateway.docx_export` resolves the URI under
  the effective user, revalidates applicant ownership and review state, embeds
  the original once, converts WebP/TIFF to PNG, and may append verified default-
  applicant `evidence:<id>` citations to a supporting-images section. It must
  never embed unreviewed, missing, malformed, or cross-applicant evidence.
- Project Word export accepts `include_images`, `applicant_id`, and optional
  `model_name`. The default `include_images=false` path must remain a direct,
  model-free export. When enabled, `deerflow.knowledge.export_images` sends
  bounded untrusted Markdown plus only `human_verified` applicant evidence to
  one text-model selection call, validates returned IDs/document indexes, and
  appends `evidence://` Markdown in memory before DOCX generation. Source
...
- MinerU ZIP extraction preserves referenced image members beside the source
  PDF in `<pdf-name>.assets/`, rewrites Markdown links, invalidates broken old
  caches, and moves/deletes the sidecar with its source PDF.
- Evidence deletion cascades through registry records, generated cards,
  unreferenced binary assets, thumbnails, and LLM-Wiki index pointers.

Focused coverage lives in `tests/test_knowledge_assets.py` and
