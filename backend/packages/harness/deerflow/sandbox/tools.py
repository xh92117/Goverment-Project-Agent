...
from deerflow.sandbox.limits import effective_write_file_max_bytes
...
def write_file_tool(
    ...
    Operators can override the cap via env var `AGENT_BASE_WRITE_FILE_MAX_BYTES`
    (0 disables the guard entirely). `DEERFLOW_WRITE_FILE_MAX_BYTES` remains a
    legacy fallback. Raising the cap risks streaming timeouts.
    ...
    """
    if not append:
        max_bytes = effective_write_file_max_bytes()
        if max_bytes > 0:
