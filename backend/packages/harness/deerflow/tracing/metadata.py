...
TRACE_ENV_ENV_VAR = "AGENT_BASE_ENV"
LEGACY_TRACE_ENV_ENV_VAR = "DEER_FLOW_ENV"
GENERIC_TRACE_ENV_ENV_VAR = "ENVIRONMENT"


def resolve_trace_environment(explicit: str | None = None) -> str | None:
    """Resolve deployment environment label for trace metadata.

    Agent Base uses ``AGENT_BASE_ENV`` as the preferred variable while keeping
    the upstream DeerFlow name and the generic ``ENVIRONMENT`` as fallbacks.
    """
    if explicit:
        return explicit
    return (
        os.environ.get(TRACE_ENV_ENV_VAR)
        or os.environ.get(LEGACY_TRACE_ENV_ENV_VAR)
        or os.environ.get(GENERIC_TRACE_ENV_ENV_VAR)
    )
