...
from deerflow.config.execution_modes_config import DEFAULT_EXECUTION_MODE, normalize_execution_mode
...
from deerflow.subagents.expert_contract import build_scoped_expert_task
...
def _apply_execution_mode_subagent_budget(config: Any, runtime: Runtime | None, app_config: "AppConfig | None") -> Any:
    mode = normalize_execution_mode(_runtime_value(runtime, "execution_mode") or DEFAULT_EXECUTION_MODE)
    if mode != "deep":
        return config
    if app_config is None:
        try:
    ...
    if budget.subagent_min_turns is not None and config.max_turns < budget.subagent_min_turns:
        overrides["max_turns"] = budget.subagent_min_turns
    if budget.subagent_min_timeout_seconds is not None and config.timeout_seconds < budget.subagent_min_timeout_seconds:
        overrides["timeout_seconds"] = budget.subagent_min_timeout_seconds
...
async def task_tool(
    ...
    config = _apply_execution_mode_subagent_budget(config, runtime, runtime_app_config)
    ...
    executor_kwargs = {
        ...
        "thread_id": thread_id,
        "trace_id": trace_id,
        "runtime_context": {key: value for key in ("project_id", "project_name", "applicant_id", "user_id", "execution_mode") if (value := _runtime_value(runtime, key)) is not None},
    }
    if resolved_app_config is not None:
    ...
    scoped_prompt = build_scoped_expert_task(
        subagent_type,
        prompt,
        getattr(runtime, "context", None),
    )
    task_id = executor.execute_async(scoped_prompt, task_id=tool_call_id)
