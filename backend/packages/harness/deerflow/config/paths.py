...
def _default_local_base_dir() -> Path:
    """Return the caller project's writable Agent Base state directory."""
    return runtime_home()
...
def make_safe_user_id(raw: str) -> str:
    ...
    IM channel ids may contain characters that
...
def _join_host_path(base: str, *parts: str) -> str:
    ...
    path form (for example ``C:\\repo\\backend\\.agent-base``).  Using
...
class Paths:
    """
    Centralized path configuration for Agent Base application data.
    ...
    BaseDir resolution (in priority order):
        ...
        2. AGENT_BASE_HOME environment variable
        3. DEER_FLOW_HOME legacy environment variable
        4. Existing `{project_root}/.agent-base` or `.deer-flow`
        5. New caller project fallback: `{project_root}/.agent-base`
    """
    ...
    def host_base_dir(self) -> Path:
        ...
        Set AGENT_BASE_HOST_BASE_DIR to the host-side path that corresponds to this
        container's base_dir so that sandbox container volume mounts work correctly.
        DEER_FLOW_HOST_BASE_DIR is still accepted as a legacy alias.

        Falls back to base_dir when the env var is not set (native/local execution).
        """
        if env := os.getenv("AGENT_BASE_HOST_BASE_DIR") or os.getenv("DEER_FLOW_HOST_BASE_DIR"):
            return Path(env)
