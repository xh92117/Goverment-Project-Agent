...
class ExtensionsConfig(BaseModel):
    ...
    def resolve_config_path(cls, config_path: str | None = None) -> Path | None:
        ...
        2. If provided `AGENT_BASE_EXTENSIONS_CONFIG_PATH` environment variable, use it.
        3. If provided legacy `DEER_FLOW_EXTENSIONS_CONFIG_PATH` environment variable, use it.
        4. Otherwise, search the caller project root for `extensions_config.json`, then `mcp_config.json`.
        5. For backward compatibility, also search legacy backend/repository-root defaults.
        ...
        Resolution order:
            ...
            2. If provided `AGENT_BASE_EXTENSIONS_CONFIG_PATH` environment variable, use it.
            3. If provided legacy `DEER_FLOW_EXTENSIONS_CONFIG_PATH` environment variable, use it.
            4. Otherwise, search the caller project root for
               `extensions_config.json`, then legacy `mcp_config.json`.
            5. Finally, search backend/repository-root defaults for monorepo compatibility.
        ...
        if config_path:
            ...
            return path
        elif os.getenv("AGENT_BASE_EXTENSIONS_CONFIG_PATH") or os.getenv("DEER_FLOW_EXTENSIONS_CONFIG_PATH"):
            env_name = "AGENT_BASE_EXTENSIONS_CONFIG_PATH" if os.getenv("AGENT_BASE_EXTENSIONS_CONFIG_PATH") else "DEER_FLOW_EXTENSIONS_CONFIG_PATH"
