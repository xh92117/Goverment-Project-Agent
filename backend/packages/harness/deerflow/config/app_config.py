...
class AppConfig(BaseModel):
    """Config for the Agent Base application."""
    ...
    execution_modes: ExecutionModesConfig = Field(default_factory=ExecutionModesConfig, description="Per-run execution mode budget configuration")
    ...
    database: DatabaseConfig = Field(
        ...
        description=format_field_description(
            "database",
            field_doc="Unified database backend for base runtime metadata (memory, sqlite, or postgres).",
        ),
    )
    ...
    def resolve_config_path(cls, config_path: str | None = None) -> Path:
        ...
        2. If provided `AGENT_BASE_CONFIG_PATH` environment variable, use it.
        3. If provided legacy `DEER_FLOW_CONFIG_PATH` environment variable, use it.
        4. Otherwise, search the caller project root.
        5. Finally, search legacy backend/repository-root defaults for monorepo compatibility.
        """
        if config_path:
            ...
            return path
        elif os.getenv("AGENT_BASE_CONFIG_PATH") or os.getenv("DEER_FLOW_CONFIG_PATH"):
            env_name = "AGENT_BASE_CONFIG_PATH" if os.getenv("AGENT_BASE_CONFIG_PATH") else "DEER_FLOW_CONFIG_PATH"
...
def get_app_config() -> AppConfig:
    """Get the Agent Base config instance.
