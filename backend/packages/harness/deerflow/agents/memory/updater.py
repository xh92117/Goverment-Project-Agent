...
from deerflow.agents.memory.project import (
    GOVERNMENT_PROJECT_AGENT_NAME,
    get_project_memory_storage,
    normalize_applicant_id,
    validate_project_id,
)
from deerflow.agents.memory.prompt import (
    MEMORY_UPDATE_PROMPT,
    PROJECT_MEMORY_UPDATE_PROMPT,
    format_conversation_for_update,
    format_user_conversation_for_project_memory,
)
...
def _parse_project_memory_update_response(response_content: Any) -> dict[str, Any]:
    """Parse the narrower project-memory response without accepting confirmed facts."""
    ...
    if not isinstance(parsed, dict):
        raise json.JSONDecodeError("Project memory response must be an object", text, 0)
    return parsed
...
class MemoryUpdater:
    ...
    def _do_update_memory_sync(
        ...
        try:
            if project_id:
                return self._update_project_memory_sync(
                    messages=messages,
                    thread_id=thread_id,
                    project_id=project_id,
                    applicant_id=applicant_id,
            ...
            if agent_name == GOVERNMENT_PROJECT_AGENT_NAME:
    ...
    def _update_project_memory_sync(
        self,
        *,
        messages: list[Any],
        thread_id: str | None,
        ...
        """Update one project's assumptions using user-authored text only."""
        safe_project_id = validate_project_id(project_id)
        safe_applicant_id = normalize_applicant_id(applicant_id)
        conversation = format_user_conversation_for_project_memory(messages)
        if not conversation.strip():
            return False

        storage = get_project_memory_storage()
        current_memory = storage.load(
            safe_project_id,
            applicant_id=safe_applicant_id,
            user_id=user_id,
        )
        prompt = PROJECT_MEMORY_UPDATE_PROMPT.format(
            project_scope=html.escape(json.dumps(current_memory.get("scope", {}), ensure_ascii=False)),
            current_memory=html.escape(json.dumps(current_memory, ensure_ascii=False, indent=2)),
            conversation=html.escape(conversation),
        )
        response = self._get_model().invoke(prompt, config={"run_name": "project_memory_agent"})
        update_data = _parse_project_memory_update_response(response.content)
        updated_memory = self._apply_project_updates(
    ...
    def _apply_project_updates(
        ...
        *,
        ...
        """Apply automatic updates; confirmed facts are immutable in this path."""
        now = utc_now_iso_z()
        existing = [item for item in current_memory.get("workingAssumptions", []) if isinstance(item, dict)]
        remove_ids = {str(item) for item in update_data.get("assumptionsToRemove", []) if isinstance(item, str) and item.strip()}
        assumptions = [item for item in existing if str(item.get("id")) not in remove_ids]
        content_keys = {key for key in (_fact_content_key(item.get("content")) for item in assumptions) if key is not None}
        new_items = update_data.get("newWorkingAssumptions")
        if isinstance(new_items, list):
            for item in new_items:
                ...
                if not isinstance(content, str) or not content.strip():
                    continue
                try:
                    confidence = _validate_confidence(float(item.get("confidence", 0.7)))
                ...
                if content_key is None or content_key in content_keys:
                    continue
                assumptions.append(
                    {
                        "id": f"pmem_{uuid.uuid4().hex[:10]}",
                        "content": normalized_content,
        ...
        current_memory["workingAssumptions"] = sorted(
            assumptions,
            key=lambda item: float(item.get("confidence", 0)),
            reverse=True,
        )[:100]
        workflow = update_data.get("workflowState")
        if isinstance(workflow, dict):
            safe_workflow = {str(key): value for key, value in workflow.items() if isinstance(value, (str, int, float, bool)) and len(str(key)) <= 80 and len(str(value)) <= 500}
            current_memory["workflowState"] = {
                **(current_memory.get("workflowState") if isinstance(current_memory.get("workflowState"), dict) else {}),
                **safe_workflow,
            }
        # Never accept confirmedFacts from model output.
        current_memory["lastUpdated"] = now
