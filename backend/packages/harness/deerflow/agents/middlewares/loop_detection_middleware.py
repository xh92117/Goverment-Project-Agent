...
class LoopDetectionMiddleware(AgentMiddleware[AgentState]):
    ...
    def _text_content(content: str | list | None) -> str:
        """Extract plain text from a mixed message content shape."""
        if content is None:
            return ""
        if isinstance(content, str):
            return content
    ...
    @staticmethod
    def _visible_hard_stop_text(warning: str | None) -> str:
        detail = warning or _HARD_STOP_MSG
        return f"{_VISIBLE_HARD_STOP_PREFIX}\n\n{detail}"
    ...
    def _record_visible_hard_stop(self, runtime: Runtime, content: str | list) -> None:
        """Persist a user-visible fallback answer for hard stops when possible."""
        context = runtime.context if runtime.context else {}
        journal = context.get("__run_journal") if isinstance(context, dict) else None
        recorder = getattr(journal, "record_assistant_message", None)
        if recorder is None:
            return
        text = self._text_content(content).strip()
        if not text:
            return
        try:
            recorder(text, caller="middleware:loop_detection")
        except Exception:
            logger.warning("Failed to record loop hard-stop fallback message", exc_info=True)
    ...
    def _apply(self, state: AgentState, runtime: Runtime) -> dict | None:
        ...
        if hard_stop:
            ...
            messages = state.get("messages", [])
            last_msg = messages[-1]
            content = self._append_text(last_msg.content, self._visible_hard_stop_text(warning))
            self._record_visible_hard_stop(runtime, content)
