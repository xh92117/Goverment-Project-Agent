...
class ClarificationMiddleware(AgentMiddleware[ClarificationMiddlewareState]):
    ...
    def _format_clarification_message(self, args: dict) -> str:
        ...
        # Type-specific prefixes. Keep them textual so frontend chat does not
        # inherit noisy emoji from internal clarification control flow.
        type_prefixes = {
            "missing_info": "需要补充信息：",
            "ambiguous_requirement": "需要确认：",
            "approach_choice": "需要选择方案：",
            "risk_confirmation": "需要确认风险：",
            "suggestion": "建议：",
        }

        prefix = type_prefixes.get(clarification_type, "需要确认：")

        # Build the message naturally
        message_parts = []

        # Add prefix and question together for a more natural flow
        if context:
            # If there's context, present it first as background
            message_parts.append(f"{prefix}{context}")
            message_parts.append(f"\n{question}")
        else:
            # Just the question with prefix
            message_parts.append(f"{prefix}{question}")
    ...
    def _handle_clarification(self, request: ToolCallRequest) -> Command:
        ...
        ai_message = AIMessage(
            id=f"{tool_message.id}:assistant",
            content=formatted_message,
            name="ask_clarification",
        )

        # Return a Command that:
        # 1. Adds the formatted tool message and displayable assistant message
        # 2. Interrupts execution by going to __end__
        return Command(
            update={"messages": [tool_message, ai_message]},
            goto=END,
        )
