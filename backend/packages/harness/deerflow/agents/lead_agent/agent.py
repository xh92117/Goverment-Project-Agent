...
from deerflow.agents.middlewares.government_research_orchestration_middleware import GovernmentResearchOrchestrationMiddleware
from deerflow.agents.middlewares.loop_detection_middleware import LoopDetectionMiddleware
from deerflow.agents.middlewares.memory_middleware import MemoryMiddleware
from deerflow.agents.middlewares.safety_finish_reason_middleware import SafetyFinishReasonMiddleware
from deerflow.agents.middlewares.subagent_limit_middleware import SubagentLimitMiddleware
from deerflow.agents.middlewares.summarization_middleware import AgentBaseSummarizationMiddleware, BeforeSummarizationHook
...
def _create_summarization_middleware(*, app_config: AppConfig | None = None) -> AgentBaseSummarizationMiddleware | None:
    ...
    # the sole entry point for AgentBaseSummarizationMiddleware, and the runtime
    # config is not expected to change after startup.
    skills_container_path = resolved_app_config.skills.container_path or "/mnt/skills"

    return AgentBaseSummarizationMiddleware(
        **kwargs,
    ...
    *,
    ...
    if subagent_enabled:
        ...
        if agent_name == "government-project-declaration":
            middlewares.append(GovernmentResearchOrchestrationMiddleware())
...
def _load_enabled_skills_for_tool_policy(available_skills: set[str] | None, *, app_config: AppConfig) -> list[Skill]:
    # Tool allowlists are enforced only for agents that explicitly declare a
    # skill whitelist. Public bundled skills may be enabled globally by default;
    # letting their allowed-tools constrain the default agent would accidentally
    # remove unrelated tools from non-specialized chats.
...
def _make_lead_agent(config: RunnableConfig, *, app_config: AppConfig):
    ...
    government_project_tools = cfg.get("government_project_tools")
    government_project_capabilities = cfg.get("government_project_capabilities")
    government_project_model_parameters = cfg.get("government_project_model_parameters")
    knowledge_answer_policy = cfg.get("knowledge_answer_policy")
    ...
    if is_bootstrap:
        ...
        return create_agent(
            ...
            system_prompt=apply_prompt_template(
                ...
                government_project_tools=government_project_tools,
                government_project_capabilities=government_project_capabilities,
                government_project_model_parameters=government_project_model_parameters,
