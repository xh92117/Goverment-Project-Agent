...
from deerflow.subagents.expert_contract import build_expert_system_contract
...
class SubagentExecutor:
    ...
    def __init__(
        ...
        Args:
            ...
            runtime_context: Safe project/user scope propagated from the parent run.
        """
        self.config = config
    ...
    async def _build_initial_state(self, task: str) -> tuple[dict[str, Any], list[BaseTool], "DeferredToolSetup"]:
        ...
        expert_contract = build_expert_system_contract(self.config.name)
        if expert_contract:
            system_parts.append(expert_contract)
