...
async def init_engine(
    ...
    *,
    ...
    pool_size: int = 5,
    sqlite_dir: str = "",
    include_auth_models: bool = False,
) -> None:
    ...
    if include_auth_models:
        import deerflow.persistence.user.model  # noqa: F401
...
async def init_engine_from_config(config, *, include_auth_models: bool = False) -> None:
    """Convenience: init engine from a DatabaseConfig object."""
    if config.backend == "memory":
        await init_engine("memory", include_auth_models=include_auth_models)
        return
    await init_engine(
