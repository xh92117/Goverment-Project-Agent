...
from deerflow.runtime.user_context import AUTO, _AutoSentinel, get_effective_user_id, resolve_user_id
...
class DbRunEventStore(RunEventStore):
    ...
    def _user_id_from_context() -> str:
        ...
        Background worker writes can run after the request context has been
        lost, so they must still be stamped with the same default bucket used
        by read paths. Otherwise refresh/history APIs filter out the rows they
        just wrote because ``resolve_user_id(AUTO)`` resolves to ``default``.
    ...
    @staticmethod
    def _apply_user_filter(stmt, resolved_user_id: str | None):
        """Apply user filtering while keeping legacy NULL-stamped events visible."""
        if resolved_user_id is None:
            return stmt
        return stmt.where(or_(RunEventRow.user_id == resolved_user_id, RunEventRow.user_id.is_(None)))
    ...
    async def list_messages(
        ...
        *,
        ...
        stmt = self._apply_user_filter(stmt, resolved_user_id)
