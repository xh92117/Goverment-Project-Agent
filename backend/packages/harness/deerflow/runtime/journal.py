...
class RunJournal(BaseCallbackHandler):
                ...
                for message in messages:
                    if isinstance(message, AIMessage):
                        self._put(
                            ...
                            category="message",
                            content=message.model_dump(),
                            metadata={"caller": "middleware:clarification"},
                        )
                        self._record_message_summary(message, caller="middleware:clarification")
    ...
    def record_assistant_message(self, content: str, *, caller: str = "lead_agent") -> None:
        """Record a user-visible assistant message produced outside a raw LLM callback."""
        text = content.strip()
        if not text:
            return
