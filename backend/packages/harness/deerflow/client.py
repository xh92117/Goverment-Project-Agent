...
from deerflow.tracing import build_tracing_callbacks, inject_langfuse_metadata, resolve_trace_environment
...
class DeerFlowClient:
        ...
        *,
        ...
        Args:
            ...
            environment: Deployment environment label that ends up in
                ...
                ``AGENT_BASE_ENV``, ``DEER_FLOW_ENV``, or ``ENVIRONMENT`` env
                vars. Pass an explicit value for programmatic callers that do
                not want env-var coupling.
        """
    ...
    def stream(
        ...
        * ``StreamBridge`` is an asyncio-queue decoupling producers from
        ...
        inject_langfuse_metadata(
            ...
            environment=resolve_trace_environment(self._environment),
        )
    ...
    def update_mcp_config(self, mcp_servers: dict[str, dict]) -> dict:
        ...
        if config_path is None:
            raise FileNotFoundError("Cannot locate extensions_config.json. Set AGENT_BASE_EXTENSIONS_CONFIG_PATH (or legacy DEER_FLOW_EXTENSIONS_CONFIG_PATH) or ensure it exists in the project root.")
