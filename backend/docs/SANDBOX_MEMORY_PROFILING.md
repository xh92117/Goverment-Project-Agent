...
compare candidates with the same Agent Base workload.
...
python scripts/sandbox_memory_profile.py \
  ...
  --selector app=agent-base-sandbox \
  --sample empty \
  --include-processes \
  --format markdown
```
...
same Agent Base workload has been measured on both the current AIO sandbox and the
candidate backend.

For an experimental provider PR, prefer `Related to #3213` unless the PR also
includes reproducible Agent Base workload data that demonstrates the target memory
