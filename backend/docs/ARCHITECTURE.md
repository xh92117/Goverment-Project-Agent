...
This document provides a comprehensive overview of the Agent Base backend architecture.
...
- `threads.py` - `/api/threads/{id}` - Local Agent Base thread data cleanup after LangGraph deletion
- `artifacts.py` - `/api/threads/{id}/artifacts` - Artifact serving

The web conversation delete flow first deletes Gateway-managed thread state through the LangGraph-compatible route, then the Gateway `threads.py` router removes Agent Base-managed filesystem data via `Paths.delete_thread_dir()`.
...
|-------------|---------------|
| `/mnt/user-data/workspace` | `backend/.agent-base/threads/{thread_id}/user-data/workspace` |
| `/mnt/user-data/uploads` | `backend/.agent-base/threads/{thread_id}/user-data/uploads` |
| `/mnt/user-data/outputs` | `backend/.agent-base/threads/{thread_id}/user-data/outputs` |
| `/mnt/skills` | `skills/` |
...
2. Gateway receives file
   - Validates file
   - Stores in .agent-base/threads/{thread_id}/user-data/uploads/
...
3. Returns response
   {
     "files": [{
       "filename": "doc.pdf",
       "path": ".agent-base/.../uploads/doc.pdf",
...
3. Gateway removes local Agent Base-managed files
   - Deletes .agent-base/threads/{thread_id}/ recursively
