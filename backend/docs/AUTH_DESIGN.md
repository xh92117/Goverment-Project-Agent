...
本文档描述 Agent Base 保留的可选本地认证模块的设计，而不是历史 RFC。它覆盖浏览器登录、API 认证、CSRF、用户隔离、首次初始化、密码重置、内部调用和升级迁移。

## 设计目标

认证模块的核心目标是为需要多用户部署的 Agent Base 项目提供本地认证与用户隔离，并让用户身份贯穿 HTTP API、LangGraph-compatible runtime、文件系统、memory、自定义 agent 和反馈数据。
...
- `token_version += 1`。
- 设置 `needs_setup=true`。
- 写入 `.agent-base/admin_initial_credentials.txt`，权限 `0600`。
...
Agent Base local auth uses Double Submit Cookie：
...
- 请求带 `X-Agent-Base-Internal-Token`。
- 同时带匹配的 CSRF cookie/header。
- 服务端识别为内部用户，`id="default"`、`system_role="internal"`。

这意味着 channel 产生的数据默认进入 `default` 用户桶。这个选择适合“平台级 bot 身份”，但不是“每个 IM 用户单独隔离”。如果后续要做到外部 IM 用户隔离，需要把外部 platform user 映射到 Agent Base user，并让 channel manager 设置对应的 scoped identity。
...
| IM 用户隔离 | channel 使用 `default` 内部用户 | 建立外部用户到 Agent Base user 的映射 |
...
| `.agent-base/data/agent_base.db` | 默认 SQLite 数据库，包含 users / threads_meta / runs / feedback 等表；已有 `deerflow.db` 仍兼容读取 |
| `.agent-base/users/{user_id}/agents/{agent_name}/` | 用户自定义 agent 配置、SOUL 和 agent memory |
| `.agent-base/admin_initial_credentials.txt` | `reset_admin` 生成的新凭据文件（0600，读完应删除） |
