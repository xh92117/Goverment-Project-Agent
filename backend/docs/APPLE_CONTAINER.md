...
Agent Base supports Apple Container as the preferred container runtime on macOS, with automatic fallback to Docker.

## Overview

Agent Base automatically detects and uses Apple Container on macOS when available, falling back to Docker when:
...
**Usage:**
```bash
# Clean up all Agent Base sandbox containers
./scripts/cleanup-containers.sh agent-base-sandbox
