...
`AGENT_BASE_ENABLE_TEST_SEED=1`) to stand up a thread with ≥2 runs and per-run
...
OPENAI_API_KEY=... OPENAI_API_BASE=<openai-compatible-endpoint>/v1 \
  AGENT_BASE_RECORD_OUT=/tmp/rec/turns.jsonl RECORD_MODEL=<model> \
...
AGENT_BASE_WRITE_GOLDEN=1 PYTHONPATH=. uv run pytest tests/test_replay_golden.py
```
