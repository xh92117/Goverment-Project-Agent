...
Agent Base supports configurable MCP servers and skills to extend its capabilities, which are loaded from a dedicated `extensions_config.json` file in the project root directory.
...
Agent Base already provides built-in file tools for thread-scoped workspace access.
Do not add an MCP filesystem server for the same Agent Base workspace. The
overlapping file tools use different path semantics, which can make LLM tool
selection and file access behavior unstable.

Agent Base does not currently adapt the MCP Roots mode for filesystem servers. In
particular, it does not publish per-thread MCP roots or map Agent Base sandbox
paths such as `/mnt/user-data/...` to paths accepted by
`@modelcontextprotocol/server-filesystem`. Use Agent Base's built-in file tools
for Agent Base workspace files.

## OAuth Support (HTTP/SSE MCP Servers)

For `http` and `sse` MCP servers, Agent Base supports OAuth token acquisition and automatic token refresh.
...
MCP servers expose tools that are automatically discovered and integrated into Agent Base agent system at runtime. Once enabled, these tools become available to agents without additional code changes.
