...
async def delete_thread_data(thread_id: str, request: Request) -> ThreadDeleteResponse:
    ...
    Cleans Agent Base-managed thread directories, removes checkpoint data,
...
async def create_thread(body: ThreadCreateRequest, request: Request) -> ThreadResponse:
    ...
    existing_record = await thread_store.get(thread_id, user_id=user_id)
...
async def patch_thread(thread_id: str, body: ThreadPatchRequest, request: Request) -> ThreadResponse:
    ...
    try:
        await thread_store.update_metadata(thread_id, body.metadata, user_id=user_id)
    except Exception:
        logger.exception("Failed to patch thread %s", sanitize_log_param(thread_id))
        raise HTTPException(status_code=500, detail="Failed to update thread")

    # Re-read to get the merged metadata + refreshed updated_at
    record = await thread_store.get(thread_id, user_id=user_id) or record
    return ThreadResponse(
        thread_id=thread_id,
...
async def get_thread(thread_id: str, request: Request) -> ThreadResponse:
    ...
    record: dict | None = await thread_store.get(thread_id, user_id=get_effective_user_id())
...
async def update_thread_state(thread_id: str, body: ThreadStateUpdateRequest, request: Request) -> ThreadStateResponse:
    ...
    if thread_store and body.values and "title" in body.values:
        ...
        if new_title:  # Skip empty strings and None
            try:
                await thread_store.update_display_name(thread_id, new_title, user_id=get_effective_user_id())
            except Exception:
