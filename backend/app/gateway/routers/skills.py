...
def _skill_name_from_markdown(content: str) -> str:
    if not content.startswith("---"):
        raise ValueError("SKILL.md must start with YAML frontmatter containing a name field.")
...
async def install_skill(
    ...
    await require_admin_user(request, resource="shared skills")
    try:
...
@router.post(
    ...
    description="Upload a SKILL.md, .skill archive, or .zip archive and install it into the skills directory.",
)
async def upload_skill(
    ...
    try:
        ...
        if suffix == ".md" or filename == SKILL_MD_FILE:
            text = content.decode("utf-8")
            skill_name = storage.validate_skill_name(_skill_name_from_markdown(text))
            storage.validate_skill_markdown_content(skill_name, text)
            scan = await scan_skill_content(text, executable=False, location=f"{skill_name}/{SKILL_MD_FILE}", app_config=config)
            if scan.decision == "block":
                raise HTTPException(status_code=400, detail=f"Security scan blocked the install: {scan.reason}")
            ...
            storage.write_custom_skill(skill_name, SKILL_MD_FILE, text)
            ...
            return SkillInstallResponse(
                ...
                message=f"Skill '{skill_name}' installed from SKILL.md.",
            )

        if suffix not in {".skill", ".zip"}:
            raise HTTPException(status_code=400, detail="Upload SKILL.md, a .skill archive, or a .zip archive.")

        with tempfile.NamedTemporaryFile(delete=False, suffix=".skill") as tmp:
            tmp.write(content)
        ...
        logger.error("Failed to upload skill: %s", e, exc_info=True)
