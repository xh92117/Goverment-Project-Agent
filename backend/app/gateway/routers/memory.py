...
class MemoryConfigUpdateRequest(BaseModel):
    """PATCH request model for editable memory configuration fields."""

    enabled: bool | None = Field(default=None, description="Whether memory is enabled")
    debounce_seconds: int | None = Field(default=None, ge=1, le=300, description="Debounce time for memory updates")
    max_facts: int | None = Field(default=None, ge=10, le=500, description="Maximum number of facts to store")
    fact_confidence_threshold: float | None = Field(default=None, ge=0.0, le=1.0, description="Minimum confidence threshold for facts")
    injection_enabled: bool | None = Field(default=None, description="Whether memory injection is enabled")
    max_injection_tokens: int | None = Field(default=None, ge=100, le=8000, description="Maximum tokens for memory injection")
    ...
    if user is not None:
        if getattr(user, "system_role", None) != "admin":
            raise HTTPException(
                ...
                detail="Admin privileges required to manage memory configuration.",
            )
        return
...
async def get_memory() -> MemoryResponse:
    ...
    Example Response:
        ```json
        {
            ...
            "user": {
                "workContext": {"summary": "Working on Agent Base project", "updatedAt": "..."},
...
async def get_memory_config_endpoint() -> MemoryConfigResponse:
    ...
    Example Response:
        ...
        {
            "enabled": true,
            "storage_path": ".agent-base/memory.json",
            "debounce_seconds": 30,
            "max_facts": 100,
...
@router.patch(
    ...
    description="Partially update editable memory system configuration fields in config.yaml.",
)
async def update_memory_config_endpoint(request: Request, body: MemoryConfigUpdateRequest) -> MemoryConfigResponse:
    """Update editable memory configuration fields and reload runtime config."""
    await _require_admin_user(request)
    config_path = AppConfig.resolve_config_path()
    data = _load_raw_config(config_path)
    memory_config = data.setdefault("memory", {})
    if not isinstance(memory_config, dict):
        raise HTTPException(status_code=500, detail="config.yaml memory field must be a mapping")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to update memory configuration.") from exc

    return _memory_config_response(reloaded.memory)
