...
async def _ensure_admin_user(app: FastAPI) -> None:
    ...
    "no-auth → with-auth" upgrade path: users who ran Agent Base without
    ...
    No SQL persistence migration is needed: the base user_id columns
    (threads_meta, runs, run_events) only come into existence
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    async with langgraph_runtime(app, startup_config):
        ...
        if config.enable_local_auth:
            # Check admin bootstrap state and migrate orphan threads after admin exists.
            # Must run AFTER langgraph_runtime so app.state.store is available for thread migration.
            await _ensure_admin_user(app)
        else:
            logger.info("Built-in local auth disabled; skipping admin bootstrap and orphan owner migration")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="Agent Base API Gateway",
        description="""
## Agent Base API Gateway

API Gateway for Agent Base - a LangGraph-compatible agent backend with sandbox execution capabilities.
...
This gateway provides runtime endpoints for agent runs plus custom endpoints for models, MCP configuration, skills, and artifacts.
        ...
        openapi_tags=[
            ...
            {
                "name": "knowledge",
                "description": "Manage knowledge-base documents for declaration workflows",
            },
            {
                "name": "projects",
                "description": "Manage project workspaces and project-scoped files",
            },
            {
                "name": "skills",
            ...
            {
                "name": "threads",
                "description": "Manage Agent Base thread-local filesystem data",
            },
            {
                "name": "agents",
            ...
            {
                "name": "channels",
                "description": "Manage IM channel integrations (DingTalk, WeChat, WeCom, Feishu)",
            },
            {
    ...
    if config.enable_local_auth:
        ...
        # CSRF: Double Submit Cookie pattern for state-changing authenticated requests
    ...
    # Proposal Drafts API is mounted at /api/proposal-drafts
    app.include_router(proposal_drafts.router)

    # Projects API is mounted at /api/projects
    app.include_router(projects.router)

    # Exports API is mounted at /api/exports
    ...
    if config.enable_local_auth:
        # Auth API is mounted at /api/v1/auth only when the built-in user system is enabled.
    ...
    async def health_check() -> dict[str, str]:
        ...
        """
        return {"status": "healthy", "service": "agent-base-gateway"}
