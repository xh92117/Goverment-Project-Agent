"""Authentication configuration for Agent Base."""
...
def _load_or_create_secret() -> str:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to read JWT secret from {secret_file}. Set AUTH_JWT_SECRET explicitly or fix AGENT_BASE_HOME/base directory permissions so Agent Base can read its persisted auth secret.") from exc
    ...
    try:
        ...
        raise RuntimeError(f"Failed to persist JWT secret to {secret_file}. Set AUTH_JWT_SECRET explicitly or fix AGENT_BASE_HOME/base directory permissions so Agent Base can store a stable auth secret.") from exc
    return secret
