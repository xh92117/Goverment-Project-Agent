...
_CONTEXT_CONFIGURABLE_KEYS: frozenset[str] = frozenset(
    {
        ...
        "government_project_model_parameters",
...
def _runtime_container_value(config: Mapping[str, Any], key: str) -> Any:
...
def _write_runtime_container_value(config: dict[str, Any], key: str, value: Any) -> None:
...
async def start_run(
    ...
    # Merge Agent Base-specific context overrides into both ``configurable`` and ``context``.
    # The ``context`` field is a custom extension for the langgraph-compat layer
    # that carries agent configuration (model_name, thinking_enabled, etc.).
    # Only agent-relevant keys are forwarded; unknown keys (e.g. thread_id) are ignored.
    merge_run_context_overrides(config, getattr(body, "context", None))
    apply_execution_mode_budget(config, run_ctx.app_config)
