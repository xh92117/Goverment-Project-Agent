...
def get_config() -> AppConfig:
    ...
    :func:`get_app_config` closes legacy upstream issue #3107 BUG-001
    ...
    *startup-time* run-events configuration the underlying ``event_store``
    ...
    async with AsyncExitStack() as stack:
        ...
        await init_engine_from_config(
            config.database,
            include_auth_models=get_gateway_config().enable_local_auth,
        )
...
def get_local_provider() -> LocalAuthProvider:
    ...
    if not get_gateway_config().enable_local_auth:
        raise RuntimeError("Built-in local auth is disabled. Set GATEWAY_ENABLE_LOCAL_AUTH=true to enable it.")
...
async def get_current_user_from_request(request: Request):
    ...
    if not get_gateway_config().enable_local_auth:
        raise HTTPException(status_code=401, detail="Built-in local auth is disabled")
