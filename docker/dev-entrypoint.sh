#!/usr/bin/env sh
#
# Agent Base gateway dev entrypoint — runs inside the docker-compose-dev gateway
...
: "${AGENT_BASE_HOME:=/app/backend/.agent-base}"
export AGENT_BASE_HOME
mkdir -p "$AGENT_BASE_HOME" /app/backend/.agent-base
...
PYTHONPATH=. exec uv run uvicorn app.gateway.app:app \
    ...
    --reload-exclude="$AGENT_BASE_HOME" \
    --reload-exclude=/app/backend/.agent-base
