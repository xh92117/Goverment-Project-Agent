"""Agent Base Sandbox Provisioner Service.
...
K8S_NAMESPACE = os.environ.get("K8S_NAMESPACE", "agent-base")
SANDBOX_IMAGE = os.environ.get(
    "SANDBOX_IMAGE",
    "enterprise-public-cn-beijing.cr.volces.com/vefaas-public/all-in-one-sandbox:latest",
)
SKILLS_HOST_PATH = os.environ.get("SKILLS_HOST_PATH", "/skills")
THREADS_HOST_PATH = os.environ.get("THREADS_HOST_PATH", "/.agent-base/threads")
        ...
        if exc.status == 404:
            ns = k8s_client.V1Namespace(
                metadata=k8s_client.V1ObjectMeta(
                    name=K8S_NAMESPACE,
                    labels={
                        "app.kubernetes.io/name": "agent-base",
...
app = FastAPI(title="Agent Base Sandbox Provisioner", lifespan=lifespan)
...
def _build_volume_mounts(thread_id: str, user_id: str = DEFAULT_USER_ID) -> list[k8s_client.V1VolumeMount]:
    ...
    if USERDATA_PVC_NAME:
        userdata_mount.sub_path = f"agent-base/users/{user_id}/threads/{thread_id}/user-data"
...
async def list_sandboxes():
    ...
    try:
        services = core_v1.list_namespaced_service(
            K8S_NAMESPACE,
            label_selector="app=agent-base-sandbox",
        )
    except ApiException as exc:
        raise HTTPException(
