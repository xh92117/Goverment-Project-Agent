# Agent Base Sandbox Provisioner

The **Sandbox Provisioner** is a FastAPI service that dynamically manages sandbox Pods in Kubernetes. It provides a REST API for the Agent Base backend to create, monitor, and destroy isolated sandbox environments for code execution.
...
2. **Pod Creation**: The provisioner creates a dedicated Pod in the `agent-base` namespace with:
...
|----------|---------|-------------|
| `K8S_NAMESPACE` | `agent-base` | Kubernetes namespace for sandbox resources |
...
| `USERS_HOST_PATH` | sibling `users` directory next to `THREADS_HOST_PATH` | **Host machine** tenant root; hostPath user data resolves to `{user_id}/threads/{thread_id}/user-data` |
| `SKILLS_PVC_NAME` | empty (use hostPath) | PVC name for skills volume; when set, sandbox Pods use PVC instead of hostPath |
| `USERDATA_PVC_NAME` | empty (use hostPath) | PVC name for user-data volume; when set, uses PVC with `subPath: agent-base/users/{user_id}/threads/{thread_id}/user-data` |
...
Older provisioner versions mounted PVC user-data from `threads/{thread_id}/user-data`. The user-scoped layout mounts from `agent-base/users/{user_id}/threads/{thread_id}/user-data`.

If an existing deployment already has PVC-backed user-data under the legacy layout, migrate the Agent Base data directory before relying on the new PVC subPath. Mount the same PVC path that the gateway uses as its Agent Base base directory, then run the existing user-isolation migration script:
...
This moves legacy `threads/{thread_id}/user-data` data under `users/<target-user-id>/threads/{thread_id}/user-data`, which matches the new provisioner PVC subPath when the gateway base directory is mounted at `agent-base/` on the PVC. Use `default` as the target user only when the legacy data should remain in the default no-auth user namespace. Run the migration while no gateway or sandbox Pods are writing to those paths.
...
3. **Kubernetes Access**:
   - The provisioner needs permissions to:
     - Create/read/delete Pods in the `agent-base` namespace
     - Create/read/delete Services in the `agent-base` namespace
     - Read Namespaces (to create `agent-base` if missing)
...
docker compose -p agent-base-dev -f docker/docker-compose-dev.yaml up -d provisioner
```
...
docker exec agent-base-provisioner curl -X POST http://localhost:8002/api/sandboxes \
  -H "Content-Type: application/json" \
  -d '{"sandbox_id":"test-001","thread_id":"thread-001","user_id":"user-001"}'

# Check sandbox status
docker exec agent-base-provisioner curl http://localhost:8002/api/sandboxes/test-001

# List all sandboxes
docker exec agent-base-provisioner curl http://localhost:8002/api/sandboxes

# Verify Pod and Service in K8s
kubectl get pod,svc -n agent-base -l sandbox-id=test-001

# Delete sandbox
docker exec agent-base-provisioner curl -X DELETE http://localhost:8002/api/sandboxes/test-001
```
...
SANDBOX_URL=$(docker exec agent-base-provisioner curl -s http://localhost:8002/api/sandboxes/test-001 | jq -r .sandbox_url)

# Test from gateway container
docker exec agent-base-gateway curl -s $SANDBOX_URL/v1/sandbox
```
...
- Verify inside container:
  ```bash
  docker exec agent-base-provisioner ls -ld /root/.kube/config
  ```
...
- Verify the paths exist on your host machine:
  ...
  ls -la /path/to/backend/.agent-base/threads
  ```
...
- Check Pod events: `kubectl describe pod sandbox-XXX -n agent-base`
...
**Solution**:
- Verify the Service exists: `kubectl get svc -n agent-base`
...
3. **Network Isolation**: Sandbox Pods run in the `agent-base` namespace but share the host's network namespace via NodePort. Consider NetworkPolicies for stricter isolation.
