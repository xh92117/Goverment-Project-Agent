...
- Standalone conversations include a right-side artifact panel. It reloads the
  current thread's generated files from the server, polls while the agent is
  running, survives a page refresh, and downloads each artifact with the
  authenticated file endpoint. The project workspace file tree also downloads
  both project-directory files and output files generated by project threads.
- Standalone and project conversation headers intentionally omit internal
  thread identifiers and model-name badges. Model selection remains available
  in the composer where it affects the next agent run.
