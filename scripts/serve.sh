#!/usr/bin/env bash
#
# serve.sh — Unified Agent Base service launcher
#
# Usage:
...
# Every Agent Base worktree (the main checkout + each linked worktree) hardcodes
...
# AGENT_BASE_ROOTS is that set of roots; processes living outside all of them
...
# True if PID has an open file/cwd under any Agent Base worktree root. The
# trailing slash keeps a sibling dir like ".../agent-base-notes" from matching
# the ".../agent-base" root.
_is_agent_base_pid() {
...
_report_reclaimed_ports() {
    ...
    for port in 8001 3000 2026; do
        for pid in $(lsof -nP -iTCP:"$port" -sTCP:LISTEN -t 2>/dev/null); do
            _is_agent_base_pid "$pid" || continue
...
_kill_repo_processes() {
    ...
    while IFS= read -r pid; do
        if [ -n "$pid" ] && _is_agent_base_pid "$pid"; then
            case " $pids " in
                *" $pid "*) ;;
                *) pids="$pids $pid" ;;
            esac
...
stop_all() {
    ...
    # so a lingering nginx (or any Agent Base process) that _kill_repo_nginx did
    ...
    ./scripts/cleanup-containers.sh agent-base-sandbox 2>/dev/null || true
...
# so pin Agent Base-owned state to the expected backend runtime directory and
# create it before uvicorn builds its reload exclude filter.
if [ -z "$AGENT_BASE_PROJECT_ROOT" ] && [ -z "$DEER_FLOW_PROJECT_ROOT" ]; then
    export AGENT_BASE_PROJECT_ROOT="$REPO_ROOT"
fi

if [ -z "$AGENT_BASE_PROJECT_ROOT" ]; then
    export AGENT_BASE_PROJECT_ROOT="$DEER_FLOW_PROJECT_ROOT"
fi

BACKEND_RUNTIME_HOME="$REPO_ROOT/backend/.agent-base"
if [ -z "$AGENT_BASE_HOME" ] && [ -z "$DEER_FLOW_HOME" ]; then
    export AGENT_BASE_HOME="$BACKEND_RUNTIME_HOME"
fi
if [ -z "$AGENT_BASE_HOME" ]; then
    export AGENT_BASE_HOME="$DEER_FLOW_HOME"
fi

mkdir -p "$AGENT_BASE_HOME" "$BACKEND_RUNTIME_HOME"
AGENT_BASE_HOME="$(cd "$AGENT_BASE_HOME" && pwd -P)"
BACKEND_RUNTIME_HOME="$(cd "$BACKEND_RUNTIME_HOME" && pwd -P)"
export AGENT_BASE_HOME
export UV_PROJECT_ENVIRONMENT="$REPO_ROOT/.venv"

# Extra flags for uvicorn
if $DEV_MODE && ! $DAEMON_MODE; then
    GATEWAY_EXTRA_FLAGS="--reload --reload-include='*.yaml' --reload-include='.env' --reload-exclude='*.pyc' --reload-exclude='__pycache__' --reload-exclude='$REPO_ROOT/backend/sandbox' --reload-exclude='$AGENT_BASE_HOME' --reload-exclude='$BACKEND_RUNTIME_HOME'"
else
    GATEWAY_EXTRA_FLAGS=""
fi
...
if ! { \
        [ -n "$AGENT_BASE_CONFIG_PATH" ] && [ -f "$AGENT_BASE_CONFIG_PATH" ] || \
        [ -n "$DEER_FLOW_CONFIG_PATH" ] && [ -f "$DEER_FLOW_CONFIG_PATH" ] || \
        [ -f backend/config.yaml ] || \
        [ -f config.yaml ]; \
    }; then
    echo "✗ No Agent Base config file found."
...
echo ""
echo "=========================================="
echo "  ✓ Agent Base is running!  [$MODE_LABEL]"
echo "=========================================="
echo ""
