...
COMPOSE_CMD="docker compose -p agent-base-dev -f docker-compose-dev.yaml"
...
init() {
    echo "=========================================="
    echo "  Agent Base Init - Pull Sandbox Image"
    echo "=========================================="
    echo ""
...
start() {
    ...
    echo "=========================================="
    echo "  Starting Agent Base Docker Development"
    echo "=========================================="
    echo ""
    ...
    # Set AGENT_BASE_ROOT for provisioner if not already set.
    if [ -z "$AGENT_BASE_ROOT" ]; then
        export AGENT_BASE_ROOT="${DEER_FLOW_ROOT:-$PROJECT_ROOT}"
        echo -e "${BLUE}Setting AGENT_BASE_ROOT=$AGENT_BASE_ROOT${NC}"
        echo ""
    fi
    ...
    if [ ! -f "$PROJECT_ROOT/config.yaml" ]; then
        if [ -f "$PROJECT_ROOT/config.example.yaml" ]; then
            ...
            echo -e "${YELLOW}  configuration before starting Agent Base.                  ${NC}"
            echo -e "${YELLOW}============================================================${NC}"
            echo ""
    ...
    echo ""
    echo "=========================================="
    echo "  Agent Base Docker is starting!"
    echo "=========================================="
    echo ""
...
stop() {
    # AGENT_BASE_ROOT is referenced in docker-compose-dev.yaml; set it before
    ...
    "$SCRIPT_DIR/cleanup-containers.sh" agent-base-sandbox 2>/dev/null || true
...
restart() {
    echo "========================================"
    echo "  Restarting Agent Base Docker Services"
    echo "========================================"
    echo ""
...
# Show help
help() {
    echo "Agent Base Docker Management Script"
    echo ""
