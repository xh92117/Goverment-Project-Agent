...
def find_config_file() -> Path | None:
    ...
    explicit = os.environ.get("AGENT_BASE_CONFIG_PATH") or os.environ.get("DEER_FLOW_CONFIG_PATH")
    if explicit:
...
def section_value(lines: list[str], section: str, key: str) -> str | None:
    ...
    Only handles the shallow shape Agent Base uses for these settings:
        database:
          backend: postgres
...
def nested_section_value(lines: list[str], section_path: str, key: str) -> str | None:
    """Return the value of a nested YAML key like ``channels.feishu.enabled``.
