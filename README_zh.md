...
- 可选本地账号：启用后提供首位管理员初始化、登录、普通用户注册、工作台访问保护和安全退出；关闭时仍可按原有方式直接进入工作台。
...
启动脚本会先预热主要前端模块，再使用系统默认浏览器打开该地址。新电脑第一次启动时，预热可能需要几十秒；
这段等待用于完成 Next.js 首次编译，避免进入页面后第一次点击模块时长时间无响应。若不希望自动打开浏览器，
可在启动命令末尾添加 `--no-open-browser`。
...
服务器运维统一执行 `make server-up`、`make server-down`、`make server-logs` 和
`make server-ps`。这些命令会恢复持久化认证密钥并补齐 Compose 所需路径，不依赖当前
Shell 中临时导出的变量。只有迁移整套数据时才修改 `AGENT_BASE_SERVER_STATE_ROOT`。

## 知识库使用

### 文本分块与目录归类

文本知识库构建采用“章节层级修复 + 未合并原始候选 + 所选知识模型联合规划 + 确定性兜底”流程。
MinerU 将“第 2 章 / 2.1 / 2.1.1 / 1）”提取成同级 Markdown 标题时，构建器会先根据编号修复父子关系。短章节不再在
模型调用前机械拼接；模型可以看到多个相邻 `source_anchor` 并决定在哪个语义边界合并或切分。

大模型只能组合连续原文单元并补充受限元数据，不能改写原文；超过上限、范围不连续、覆盖不完整或字段越界的结果会被拒绝。
当整个独立小节本身低于 500 字时，允许模型保留完整小节并做语义归类，之后再由发布前后处理决定是否与结构相容的邻块合并。
单批失败会先重试，连续多批失败才会熔断；回退块会恢复确定性短块合并，不会因调整模型执行顺序而产生更多碎块。

- 默认分块下限、目标和上限分别为 500、1600、3200 字。模型处理后还会执行一次短块
  后处理，将兼容的短叶子块和父级摘要合并，并保留全部 `source_anchors`。
- 研究目标、创新点、考核指标、评价结论、申报条件和参考文献等不可再拆的独立小节可标记为
  `atomic_short`，不会为了达到长度机械拼接到不相关章节。
- 待整理文件先分别给文件名/路径、Markdown 标题和正文证据计分。已高置信命中的类别或领域保持不变；仅对
  仍为“未分类/通用”的维度调用同一 `knowledge_model` 语义复核。模型只能从配置的类别和领域枚举中选择；低置信、越界或调用失败时保留规则结果。
- 归档结果记录 `category_strategy` / `domain_strategy`、模型、置信度、理由和回退警告，便于判断物理目录来自规则还是语义分类。
- 构建质量报告默认统计低于 500 字的非原子短块；多块文档中低于 120 字的碎片视为严重问题，
  短块占比超过 5% 会告警。单篇本身很短但完整的资料只告警，不阻断兼容构建流程。

本轮配置模板版本为 17。已有 `config.yaml` 可在项目根目录执行 `make config-upgrade` 合并
`max_call_attempts`、`circuit_breaker_failures` 和新的质量阈值；升级后再重建索引。
...
- `proposal-writer`：申报书核心章节撰写。
- `budget-analyst`：预算编制和费用说明辅助。
- `compliance-reviewer`：指南适配、模板完整性、证据和预算合规审查。
