...
文本知识库构建采用“确定性解析与兜底 + 所选知识模型规划 + 发布前规则校验”的组合流程。
大模型只能组合连续原文单元并补充受限元数据，不能改写原文；模型给出的块低于或超过配置
长度、范围不连续、覆盖不完整或字段越界时会被拒绝。单批失败会先重试，连续多批失败才会
熔断并对剩余材料使用规则分块，因此一次临时接口失败不会使整次构建都退化。
...
- 待整理文件不再按正文中首先出现的关键词直接归类，而是分别给文件名/路径、Markdown 标题
  和正文证据计分；专有短语的权重大于“标准”“成果”“检测”等弱词。默认目录规则已补充
  “技术资料”和“无损检测”，降低技术原理资料被误放到“团队成果”等目录的概率。
