...
可在根目录 `.env` 中设置 `GP_AGENT_HOME` 修改总目录，也可以通过
`AGENT_BASE_HOME`、`GOVERNMENT_PROJECT_WORKSPACE_ROOT`、
`AGENT_BASE_KNOWLEDGE_ROOT`、`GOVERNMENT_PROJECT_DRAFTS_ROOT`、
`GOVERNMENT_PROJECT_PROJECTS_ROOT` 和 `GOVERNMENT_PROJECT_LOG_ROOT`
分别覆盖子目录。前端“设置 → 存储目录”保存时会安全更新这些 `.env` 项，重启后生效。
路径修改不会自动搬移旧数据；如需保留历史线程、知识库或草稿，请停止服务后再复制目录。
...
2. 在“项目”页面新建项目，并明确选择工作路径：可使用系统默认项目目录，也可通过
   文件夹选择器指定本机目录。选择器会以前置的 Windows 系统窗口打开，网页通过短轮询
   等待结果；未选择路径前不会创建项目。
